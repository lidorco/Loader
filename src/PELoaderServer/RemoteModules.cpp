...
{
	HMODULE hModule = GetRemoteModuleHandle(hProcess, "kernel32.dll");
	FARPROC RemoteLoadLibraryAddress = GetRemoteProcAddress(hProcess, hModule, "LoadLibraryA");
	if (!hModule || !RemoteLoadLibraryAddress)
	{
		return NULL;
	}

	LPVOID ModuleNameBufferInRemoteProcess = VirtualAllocEx(hProcess, NULL,
		strlen(lpModuleName) + 1, MEM_COMMIT | MEM_RESERVE, PAGE_READWRITE);

	WriteProcessMemory(hProcess, ModuleNameBufferInRemoteProcess, lpModuleName, strlen(lpModuleName) + 1, NULL);
	
	DWORD ThreadId = 0;
	HANDLE hThread = CreateRemoteThread(hProcess, NULL, NULL, (LPTHREAD_START_ROUTINE)RemoteLoadLibraryAddress,
		ModuleNameBufferInRemoteProcess, NULL, &ThreadId);

	WaitForSingleObject(hThread, 10000);
	VirtualFreeEx(hProcess, ModuleNameBufferInRemoteProcess, NULL, MEM_RELEASE);
	return GetRemoteModuleHandle(hProcess, lpModuleName);
}